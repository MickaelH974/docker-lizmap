...
RUN apt-get install -y python-simplejson xauth htop nano curl ntp ntpdate python-software-properties git wget unzip \
    ...
    libapache2-mod-php5 qgis-server apache2-mpm-prefork xvfb --force-yes
...
# Copy xvfb configuration for fake xserver
...
CMD /start.sh
ADD startfakex.sh /startfakex.sh
RUN chmod 0755 /startfakex.sh
